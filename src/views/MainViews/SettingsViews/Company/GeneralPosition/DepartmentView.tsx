...
export function AddDepartmentScreen(props:any)
{
    ...
    const [selectedGeneralPositionObj, setSelectedGeneralPositionObj] = useState(Object);

    const [GeneraPositionArray, setGeneralPositionArray] = useState(new Array);

    const [GeneralPositionName, setGeneralPositionName] = useState('');
    ...
    async function addDepartmentReq(){
        try{
        const status = await addGeneralPosition(selectedScoupeObj.ID, GeneralPositionName);
    ...
    async function callbackGeneralPosition(){
        try{
        const response = await getScoupes();
        setScoupeArray(response)
        setGeneralPositionArray(response.find((item:any)=>item.ID===selectedScoupeObj.ID).GeneralPosition);
        }
        catch{}
    }
    ...
    function renderItem(emp:any){
        ...
        return (<TouchableOpacity onPress={()=>setSelectedGeneralPositionObj(null)}><DepartmentItemFull Department={emp.item} callbackDepartment = {callbackDepartment}/></TouchableOpacity>)
        
        return (<TouchableOpacity onPress={()=>setSelectedGeneralPositionObj(emp.index)}><DepartmentItem Department={emp.item}  /></TouchableOpacity>)
    };
        ...
        <View>
            ...
            >
                ...
                renderItem = {(emp)=><TouchableOpacity onPress={()=>{
                                                        ...
                                                        if(emp.item.GeneralPosition)
                                                        {
                                                        setGeneralPositionArray(emp.item.GeneralPosition);
